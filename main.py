...
def read_data(filename):
    ...
    with open(filename, mode='r', encoding='utf8') as f:
        data=f.readlines()
        data=[ligne.strip() for ligne in data]
    return data
...
def cherche1(mots, start, stop, n):
    ...
    ens_mots=set()
    for elt in mots:
        if elt.startswith(start) and elt.endswith(stop) and len(elt)==n:
            ens_mots.add(elt)
    return ens_mots
...
def main():
    ...
    mab17ez = cherche2(mots, 'a', 'b', 'z', 16, 16)
    print(mab17ez)
    print(cherche2(ensemble_mots(FILENAME),CONSONNES,'ç',VOYELLES,8,12))
